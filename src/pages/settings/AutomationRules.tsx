...
export default function AutomationRules() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('automation_rules').select(`
          *,
          event:events(name),
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('events').select('id, name, brand_id').eq('status', 'active');
      if (error) throw error;
      return data;
    }
  });
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('templates').select('id, name, type');
      if (error) throw error;
      return data;
    }
  });
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('locations').select('id, name, brand_id');
      if (error) throw error;
      return data;
    }
  });
  ...
  const filteredTemplates = templates.filter(t => form.channel === 'email' && t.type === 'email' || form.channel === 'sms' && t.type === 'sms' || form.channel === 'both');
  const filteredLocations = locations.filter(l => form.brand_ids.length === 0 || form.brand_ids.includes(l.brand_id || ''));
              ...
              <TableBody>
                {isLoading ? <TableRow>
                    ...
                    <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                      ...
                      <TableCell>
                        {rule.delay_hours === 0 ? <span className="text-muted-foreground">Immediate</span> : <span className="flex items-center gap-1">
                            <Clock className="h-3 w-3" />
                            {Math.floor(rule.delay_hours / 24)}d
                          </span>}
                      </TableCell>
                      <TableCell>
                        <Switch checked={rule.status === 'active'} onCheckedChange={checked => toggleMutation.mutate({
                    id: rule.id,
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem className="text-destructive" onClick={() => deleteMutation.mutate(rule.id)}>
          ...
          <DialogContent className="sm:max-w-[700px] max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                ...form,
                name: e.target.value
              })} placeholder="e.g., Promoter Thank You" className={cn(errors.name && 'border-destructive')} />
              ...
              <div className="space-y-2">
                ...
                <div className={cn("border rounded-lg p-3 space-y-2 max-h-32 overflow-y-auto", errors.event_ids && 'border-destructive')}>
                  {events.length === 0 ? <p className="text-sm text-muted-foreground">No active events found</p> : events.map(e => <div key={e.id} className="flex items-center gap-2">
                      <Checkbox id={`event-${e.id}`} checked={form.event_ids.includes(e.id)} onCheckedChange={() => setForm({
                    ...form,
                    event_ids: toggleArrayItem(form.event_ids, e.id)
                  })} />
              ...
              <div className="space-y-2">
                ...
                <div className={cn("border rounded-lg p-3 space-y-2 max-h-32 overflow-y-auto", errors.brand_ids && 'border-destructive')}>
                  {brands.length === 0 ? <p className="text-sm text-muted-foreground">No brands found</p> : brands.map(b => <div key={b.id} className="flex items-center gap-2">
                      <Checkbox id={`brand-${b.id}`} checked={form.brand_ids.includes(b.id)} onCheckedChange={() => {
              ...
              <div className="space-y-2">
                ...
                <div className={cn("border rounded-lg p-3 space-y-2 max-h-32 overflow-y-auto", errors.location_ids && 'border-destructive')}>
                  {filteredLocations.length === 0 ? <p className="text-sm text-muted-foreground">{form.brand_ids.length === 0 ? 'Select brands first' : 'No locations found'}</p> : filteredLocations.map(l => <div key={l.id} className="flex items-center gap-2">
                      <Checkbox id={`location-${l.id}`} checked={form.location_ids.includes(l.id)} onCheckedChange={() => setForm({
                    ...form,
                    location_ids: toggleArrayItem(form.location_ids, l.id)
                  })} />
              ...
              <div className="space-y-2">
                ...
                <div className={cn("flex flex-wrap gap-2", errors.trigger_groups && 'ring-1 ring-destructive rounded-lg p-2')}>
                  ...
                  className: 'border-promoter/30 data-[state=checked]:bg-promoter-bg data-[state=checked]:text-promoter'
                }, {
                  id: 'passive',
                  label: 'Passives (7-8)',
                  className: 'border-passive/30 data-[state=checked]:bg-passive-bg data-[state=checked]:text-passive'
                }, {
                  id: 'detractor',
                  label: 'Detractors (0-6)',
                  className: 'border-detractor/30 data-[state=checked]:bg-detractor-bg data-[state=checked]:text-detractor'
                }].map(tg => <label key={tg.id} className="flex items-center gap-2 cursor-pointer">
                      <Checkbox id={`trigger-${tg.id}`} checked={form.trigger_groups.includes(tg.id)} onCheckedChange={() => setForm({
                    ...form,
                    trigger_groups: toggleArrayItem(form.trigger_groups, tg.id)
                  })} />
              ...
              <div className="space-y-2">
                ...
                <div className={cn("flex flex-wrap gap-4", errors.feedback_conditions && 'ring-1 ring-destructive rounded-lg p-2')}>
                  ...
                  id: 'either',
                  label: 'Either'
                }].map(fc => <label key={fc.id} className="flex items-center gap-2 cursor-pointer">
                      <Checkbox id={`feedback-${fc.id}`} checked={form.feedback_conditions.includes(fc.id)} onCheckedChange={() => setForm({
                    ...form,
                    feedback_conditions: toggleArrayItem(form.feedback_conditions, fc.id)
                  })} />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={form.channel} onValueChange={v => setForm({
                  ...form,
                  channel: v,
                  template_id: ''
                })}>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={form.template_id || "__none__"} onValueChange={v => setForm({
                  ...form,
                  template_id: v === "__none__" ? "" : v
                })}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" min={0} value={form.delay_days} onChange={e => setForm({
                  ...form,
